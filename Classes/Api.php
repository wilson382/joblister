...
class Api{
	...
	public function validateToken($token_name,$token_id,$token_password){
		...
		$db->query("SELECT * FROM `api_tokens`WHERE `token_name`=:token_name
		AND `token_id`=:token_id AND `token_password`=:token_password;");

		$db->bind(":token_name",$token_name);
...
if (isset($_GET['name']) && isset($_GET['id']) && isset($_GET['pw'])){
	$token_name=$_GET['name'];
	$token_id=$_GET['id'];
	$token_password=$_GET['pw'];
	$api = new Api;
	if ($api->validateToken($token_name,$token_id,$token_password)){
